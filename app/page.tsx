        ...
        {posts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {posts.map((post) => {
              ...
              return (
                <Link href={`/question/${post.id}`} key={post.id} className="block h-full">
                  <article className="group relative flex h-full flex-col overflow-hidden rounded-md border border-white/10 bg-[#0a0a0a] p-4 transition-colors hover:border-white/20">
                    ...
                    <div className="mb-3 flex-1">
                      <p className="line-clamp-3 text-sm leading-relaxed text-slate-400">{post.content}</p>
                    </div>
